...
// ===== OPTIMIZACIÓN: Animaciones adaptativas =====
...
const FAQ_ITEMS = [
    { q: "¿Cuánto tiempo lleva desarrollar una identidad de marca completa?", a: "Nuestro proceso de branding estratégico toma entre 4 y 6 semanas. Incluye investigación, naming, diseño de logo, manual de identidad y guía de storytelling." },
    { q: "¿Hacen sitios web para emprendedores sin experiencia técnica?", a: "Sí. Diseñamos plataformas intuitivas y te entregamos capacitación + acceso fácil. Además, ofrecemos soporte continuo para que nunca estés solo." },
    { q: "¿Ofrecen servicios solo en Argentina?", a: "Trabajamos con clientes de toda Latinoamérica. Nuestros servicios son 100% online, con acompañamiento en español y horarios flexibles." },
    { q: "¿Qué incluye el servicio de SEO?", a: "Optimización técnica, investigación de palabras clave, contenido estratégico, velocidad del sitio y seguimiento mensual de posicionamiento y tráfico." }
];
...
export default function ServicesContent() {
    ...
    // ===== OPTIMIZACIÓN: useScroll solo si no es reduced-motion =====
    const scrollData = useScroll();
    const yBackground = useTransform(scrollData.scrollYProgress, [0, 1], ["0%", "30%"]);
    ...
    const filteredServices = useMemo(() => {
        if (activeCategory === 'Todos') return SERVICES_DATA;
        return SERVICES_DATA.filter(s => s.categoria === activeCategory);
    }, [activeCategory]);
    ...
    // ===== OPTIMIZACIÓN: Background simplificado por dispositivo =====
    const BackgroundEffects = useCallback(() => {
        if (reducedMotion) {
            return <div className="fixed inset-0 bg-gradient-to-b from-gray-950 via-gray-900 to-black pointer-events-none" />;
        }
        ...
        <main className={`min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-black text-white overflow-x-hidden ${playfair.variable} ${inter.className}`}>
            ...
            {!isMobile && !reducedMotion && (
                <motion.div
                    ...
                    style={{ scaleX: scrollData.scrollYProgress }}
                />
            )}
            ...
            <section ref={sectionRef} className="relative container mx-auto px-6 py-20 md:py-28 lg:py-32">
                ...
                >
                    ...
                    <motion.h1
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: reducedMotion ? 0.3 : 0.7, delay: reducedMotion ? 0 : 0.3 }}
                    >
                        Transforma tu{" "}
                    ...
                    <motion.p
                        ...
                        initial={{ opacity: 0, y: 30 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: reducedMotion ? 0.3 : 0.7, delay: reducedMotion ? 0 : 0.4 }}
                    >
                    ...
                    <motion.div
                        ...
                        initial={{ opacity: 0, y: 30 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: reducedMotion ? 0.3 : 0.7, delay: reducedMotion ? 0 : 0.5 }}
                    >
                        <a
                            href="/contact"
                            ...
                            >
                                ...
                                <div className={`relative bg-white/5 backdrop-blur-xl rounded-3xl p-7 border border-white/10 hover:border-amber-500/50 transition-all duration-500 hover:-translate-y-2 ${service.featured ? 'ring-2 ring-amber-500/30' : ''}`}>
                                    ...
                                    <motion.div
                                        ...
                                        whileHover={isMobile || reducedMotion ? undefined : { rotate: 5 }}
                                    >
                ...
                <section className="py-16 px-6">
                    <div className="max-w-6xl mx-auto">
                        ...
                        <motion.div
                            ...
                            viewport={{ once: true }}
                        >
                            {VALOR_DESTACADO_ITEMS.map((item, i) => (
                                <motion.div key={i} variants={animations.fadeInUp}>
                                    <ValorDestacado
                                        titulo={item.titulo}
            ...
            {/* Sticky CTA bar para desktop - solo si no es reduced-motion */}
            {!reducedMotion && (
                <motion.div
                    ...
                    initial={{ y: 100 }}
                    animate={{ y: scrollData.scrollYProgress.get() > 0.3 ? 0 : 100 }}
